...
export function createSteleTokenBonusEvent(
  challengeId: BigInt,
  user: Address,
  action: string,
  amount: BigInt
): SteleTokenBonus {
  let steleTokenBonusEvent = changetype<SteleTokenBonus>(newMockEvent())

  steleTokenBonusEvent.parameters = new Array()

  steleTokenBonusEvent.parameters.push(
    new ethereum.EventParam(
      "challengeId",
      ethereum.Value.fromUnsignedBigInt(challengeId)
    )
